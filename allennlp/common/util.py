...
def params_from_file(path: str, fmt: str = 'auto'):
    fmt = fmt.lower().strip()
    process_funcs = {}
    ...
    def load_auto(p):
        for func in [load_yaml, load_jsonnet, load_json]:
            try:
                return func(p)
            except:
                pass 
        
    process_funcs['json'] = load_json
    process_funcs['jsonnet'] = load_jsonnet
    process_funcs['yaml'] = load_yaml
    process_funcs['auto'] = load_auto

    param_dict = None
    func = process_funcs.get(fmt, None)
    if func:
        param_dict = func(path)
    assert param_dict, f'load params from {path} failed! '
