...
class LSTMModel(Model):
    def __init__(self,
        ...
        self.rnn = nn.LSTM( input_size = n_wemb, hidden_size=n_hidden, 
                            num_layers=n_layer, batch_first=True, dropout=dropout)
        ...
        if tie_weights:
            if n_hidden != n_wemb:
                raise ValueError('When using the tied flag, n_hidden must be equal to n_wemb')
    ...
    def forward(self, input_tensor):
        ...
        mask = input_tensor[:,2:] != self.vocab.PAD # [batch_size, seq_len-2 ]

        targets = input_tensor[:, 1:-1].reshape((batch_size *(seq_len-2),)) # [batch_size *(seq_len-2) ]

        emb = self.dropout(self.embedder(input_tensor)) #[batch_size, seq_len, n_wemb]
        ...
        loss_inst =  (loss_token * mask.float()).sum(dim = -1)/ mask.sum(dim=-1).float()
...
class GPT2Wrap(Model):
    ...
    def forward(self, input_tensor):
        ...
        output = self.gpt2_model(input_ids = input_tensor, attention_mask = attention_mask,
        ...
        loss_token = F.cross_entropy(shift_logits, shift_targets, reduction='none').view(bs, seq_len)
